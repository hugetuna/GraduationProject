...
public class PickManager : MonoBehaviour
{
    ...
    public AudioClip photoSound; // 點擊角色圖片的音效
    public AudioClip confirmSound; // 確認隊伍編排的音效
    void Start()
    {
        ...
        // 進入該場景時，設定背景音樂
        if (bgm != null) AudioManager.Instance.SetMusic(bgm);
    }
        ...
        for (int i = 0; i < whoPickedList.Count; i++)
        {
            if ((int)whoPickedList[i].pickedIdol == whoToPick)
            {
                ...
                if (!tempPicked.isPicked && pickedIdolCount != 3)//未選取時且未選滿三人
                {
                    ...
                    // 播放點擊角色圖片的音效
                    if(photoSound != null) AudioManager.Instance.PlaySFX(photoSound);
                }
                else if (tempPicked.isPicked)//已選取時
                {
                    ...
                    // 播放點擊角色圖片的音效
                    if(photoSound != null) AudioManager.Instance.PlaySFX(photoSound, setPitch: 0.8f);
                }
                else
                {
                    //已選滿三人時不做任何事
    ...
    public void ConfirmPick()
    {
        ...
        {
            // 播放確認隊伍編排的音效
            if(confirmSound != null) AudioManager.Instance.PlaySFX(confirmSound);
